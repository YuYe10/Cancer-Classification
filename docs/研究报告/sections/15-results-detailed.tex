...
本节基于实验日志汇总文件中记录的 41 组实验完整数据，对各类融合策略进行逐项详细分析。所有数值均来自 repeated CV 的 fold-level 聚合统计，确保正文数据与实验日志完全一致。
...
\hline
RNA (SVM) & 0.8978 & 0.8844 & 0.8899 & 50 & SVM \\
RNA (XGB) & 0.8764 & 0.8781 & 0.8752 & 50 & XGBoost \\
\hline
\end{tabular}
\end{table}
...
\textbf{配置} & \textbf{Acc Mean} & \textbf{Balanced Acc} & \textbf{Macro F1} & $n_{fold}$ & \textbf{分类器} \\
\hline
Meth (XGB) & 0.8367 & 0.8310 & 0.8244 & 50 & XGBoost \\
\hline
\end{tabular}
\end{table}
...
\hline
Concat (SVM) & 0.9064 & 0.9056 & 0.9003 & 50 & 最优配置 \\
Concat (XGB) & 0.8982 & 0.8923 & 0.8880 & 50 & XGB baseline \\
Concat (RF) & 0.9000 & 0.8932 & 0.8949 & 15 & RF 分类器 \\
\hline
\end{tabular}
\end{table}
...
\hline
\textbf{Base} & \textbf{Meta} & \textbf{Acc} & \textbf{Bal-Acc} & \textbf{Macro-F1} \\
\hline
...
\begin{itemize}
    ...
    \item 内层 CV 折数（5 折）在小样本条件下导致 base model 训练数据不足；
...
\begin{tabular}{|ll|ccc|}
\hline
\textbf{参数} & \textbf{值} & \textbf{Acc} & \textbf{Bal-Acc} & \textbf{Macro-F1} \\
\hline
